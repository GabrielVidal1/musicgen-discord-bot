...
class general(commands.Cog):
    ...
    async def ping(self, inter: disnake.ApplicationCommandInteraction):
        try:
            ...
            embed.set_footer(
                text=f"Command executed by {inter.author}",
    ...
    async def check(inter):
        try:
            ...
            await inter.send(
                embed=errors.create_error_embed(f"Error sending check command: {e}")
            )
    ...
    async def on_member_join(self, member):
        try:
            ...
            embed = disnake.Embed(
                ...
                description=f"{member.guild.name}! We hope you enjoy your stay here!",
                color=config.Success(),
            )
            embed.add_field(
                name="\nUser Info",
    ...
    async def userinfo(
        ...
        try:
            ...
            embed.add_field(
                name="User Top Role", value=f"{member.top_role.mention}", inline=False
            )
            embed.add_field(
                name="User Roles",
            ...
            embed.add_field(
                ...
                value=f"```{', '.join([perm for perm, value in member.guild_permissions if value])}```",
                inline=False,
            )
            embed.add_field(
                name="User Status", value=f"```{member.status}```", inline=False
            )
            embed.add_field(
                name="User Boosting",
                value=f"```{member.premium_since}```",
                inline=False,
            )
            embed.add_field(
                name="User Avatar",
            ...
            embed.add_field(
                name="User Profile",
                value=f"[Click Here](https://discord.com/users/{member.id})",
                inline=False,
            )
            embed.add_field(
                name="User Mention", value=f"{member.mention}", inline=False
            )
            embed.set_thumbnail(url=member.avatar.url)
            embed.set_footer(
                text=f"Requested by {inter.author}", icon_url=inter.author.avatar.url
            ...
            await inter.send(
                embed=errors.create_error_embed(f"Error sending userinfo command: {e}")
            )
    ...
    async def botinfo(self, inter: disnake.ApplicationCommandInteraction):
        try:
            ...
            await inter.send(
                embed=errors.create_error_embed(f"Error sending botinfo command: {e}")
            )
    ...
    async def invite(self, inter: disnake.ApplicationCommandInteraction):
        try:
            ...
            await inter.send(
                embed=errors.create_error_embed(f"Error sending invite command: {e}")
            )
